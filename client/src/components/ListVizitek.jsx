...
export function ListVizitek() {
  ...
  useEffect(() => {
    async function fetchData() {
      var data = await fetch(fetchURL + "/lecturers/").then(res => {
        return res.json();
      }).catch((error) => {
        console.log(error);
      });
  ...
  useEffect(() => {
    async function fetchData() {
      var data = await fetch(fetchURL + "/tag").then(res => {
        return res.json();
      }).catch((error) => {
        console.log(error);
      });
  ...
  useEffect(() => {
    async function fetchData() {
      var data = await fetch(fetchURL + "/location").then(res => {
        return res.json();
      }).catch((error) => {
        console.log(error);
      });
  ...
  const filteredDATA = data.filter((node) => {
    ...
    const priceMatch = (!(filterPrice[1] + 1 <= filterPrice[0] - 1) && filterPrice[1] + 1 >= filterPrice[0] - 1) && filterPrice.every(() => node.price_per_hour <= (filterPrice[1]) && node.price_per_hour >= (filterPrice[0]));
        ...
        {data.length === 0 ?
          <FadeInView className="self-center w-full">
            ...
            <div className='mt-10 mb-5 flex flex-col justify-between w-full md:w-full flex-wrap'>
              ...
              <div className='md:min-w-[20rem] sm:min-w-[10rem] min-w-[30rem] max-w-6xl relative gap-8'>
